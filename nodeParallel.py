...
class NodeParallel:
    def __init__(self, game, args, board, jumps, parent=None, action_taken=None, prior=0, visit_count=0, level=0):
        ...
        self.visit_count = [visit_count] * self.args['num_parallel_games']
        self.level = level

        self.children: list[NodeParallel] = []
        self.value_sum = [0] * self.args['num_parallel_games']
    ...
    def __str__(self, i):
        ...
        return \
            f"""
{indent}{"-"*60}
{indent}Level: {self.level}, N: {self.visit_count[i]}, val: {self.value_sum[i]:.3f}, prior: {self.prior:.3f}, uct:{self.parent.get_ucb(self, i) if self.parent else None}, weight: {self.search_weight}, num_children: {len(self.children)}
{board_repr}
...
"""
    ...
    def select(self, i):
        if self.jumps == []:
            ...
            for child in self.children:
                ucb = self.get_ucb(child, i)
                if ucb > best_ucb:
                    best_child = child
                    best_ucb = ucb
            return best_child

    def get_ucb(self, child, i):
        if child.visit_count[i] == 0:
            q_value = float("inf")
        else:
            q_value = 1 - ((child.value_sum[i] / child.visit_count[i]) + 1) / 2
        return q_value + self.args['C'] * (math.sqrt(self.visit_count[i]) / (child.visit_count[i] + 1)) * child.prior
    ...
    def backpropagate(self, value, i):
        self.value_sum[i] += value
        self.visit_count[i] += 1
        if self.jumps == []:
            value = self.game.get_opponent_value(value)
        if self.parent is not None:
            self.parent.backpropagate(value, i)
